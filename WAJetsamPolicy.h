...
#define WAJ_TARGET_LIMIT_MB 40

static inline bool waj_is_whatsapp_service_path(const char *path) {
...
static inline int32_t waj_clamp_positive_limit(int32_t value) {
    return value > 0 && value < WAJ_TARGET_LIMIT_MB ? WAJ_TARGET_LIMIT_MB : value;
}

static inline uint32_t waj_clamp_flags_limit(uint32_t value) {
    return value > 0 && value < WAJ_TARGET_LIMIT_MB ? WAJ_TARGET_LIMIT_MB : value;
}
