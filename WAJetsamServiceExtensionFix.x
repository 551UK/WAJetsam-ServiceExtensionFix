...
// Made by 551
// runningboardd component: keep WhatsApp ServiceExtension
...
static const char *waj_basename(const char *path) {
    if (!path) return NULL;
    const char *slash = strrchr(path, '/');
    return slash ? slash + 1 : path;
}

static bool waj_current_executable_is(const char *name) {
    char path[PROC_PIDPATHINFO_MAXSIZE] = {0};
    uint32_t size = sizeof(path);
    if (_NSGetExecutablePath(path, &size) != 0) return false;
    const char *base = waj_basename(path);
    return base && strcmp(base, name) == 0;
}

static bool waj_target_pid(int32_t pid) {
    if (pid < 2) return false;
    char path[PROC_PIDPATHINFO_MAXSIZE] = {0};
    int n = proc_pidpath(pid, path, PROC_PIDPATHINFO_MAXSIZE);
    return n > 0 && waj_is_whatsapp_service_path(path);
}

static int waj_hooked_memorystatus_control(uint32_t command,
                                           int32_t pid,
                                           uint32_t flags,
                                           void *buffer,
                                           size_t buffersize) {
    ...
    if (command == MEMORYSTATUS_CMD_SET_MEMLIMIT_PROPERTIES) {
        ...
        modified.memlimit_active = waj_clamp_positive_limit(modified.memlimit_active);
        modified.memlimit_inactive = waj_clamp_positive_limit(modified.memlimit_inactive);
        new_buffer = &modified;
    } else {
        new_flags = waj_clamp_flags_limit(flags);
    }
...
%ctor {
    @autoreleasepool {
        if (!waj_current_executable_is("runningboardd")) return;

        MSHookFunction((void *)memorystatus_control,
                       (void *)waj_hooked_memorystatus_control,
